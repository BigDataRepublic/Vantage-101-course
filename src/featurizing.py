...
class Featurizer(Runner):
    def __init__(self):
        self.categorical = None
        self.numerical = None

    def run(self, X):
        if self.categorical is None and self.numerical is None:
            self._set_cat_and_num_cols(X)
    ...
    def _set_cat_and_num_cols(self, X_train):
        # Defining our categorical and numerical variables
        self.categorical = [var for var in X_train.columns if X_train[var].dtype == "O"]
        self.numerical = [var for var in X_train.columns if X_train[var].dtype != "O"]
